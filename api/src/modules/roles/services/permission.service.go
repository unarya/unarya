...
func CheckPermission(roleID uuid.UUID, permissionData dto.PermissionInterface) bool {
	...
	var exists bool
	err := db.Model(&roles.RolePermission{}).
		Select("1").
...
func CreatePermission(resource, action string) (bool, error) {
	db := config.DB

	var exists bool
	err := db.Model(roles.Permission{}).Select("name").Where("resource = ? && action = ?", resource, action).First(&roles.Permission{}).Scan(&exists).Error
	if err == nil || exists {
...
func AddPermissionsToRole(roleID uuid.UUID, permissionIDs []uuid.UUID) (map[string]interface{}, error) {
	...
	if err := db.Where("role_id = ?", roleID).Delete(&roles.RolePermission{}).Error; err != nil {
	...
	var rolePermissions []roles.RolePermission
