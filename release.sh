...
#   github.com/unarya/unarya v0.0.2-alpha.3
#   → github.com/unarya/unarya v0.0.2-alpha.4
for modfile in $(find cmd -type f -name "go.mod"); do
    if grep -q "github.com/unarya/unarya" "$modfile"; then
        log "Updating $modfile ..."
        # Try replacing version
        if ! sed -i.bak -E "s#(github\.com/unarya/unarya)[[:space:]]+v[0-9]+\.[0-9]+\.[0-9]+(-[a-zA-Z]+\.[0-9]+)?#\1 ${TAG}#g" "$modfile"; then
